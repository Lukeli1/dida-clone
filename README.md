...
![版本](https://img.shields.io/badge/version-1.46.0-blue)
...
为避免静态界面图与桌面端迭代脱节，本仓库当前以功能清单、使用说明和发布说明作为功能依据，不附带易过期的界面截图。
...
- 子任务：支持一层任务拆解，独立勾选完成，顶层任务右键可直接打开“添加子任务”输入框
- **回收站与可恢复删除**（v1.43.0）：删除任务会移入回收站，可恢复任务及同次删除的子任务；归档用于收纳，不会进入回收站
...
- 月视图：悬停"+"按钮快速添加，双击打开详细弹窗；支持跨日期拖拽选择范围后创建跨日任务
- 周视图：单击快速添加，拖拽时间段创建任务；支持跨日期、跨时间拖拽创建，边缘自动滚动
- 日视图：同周视图，支持时间段任务块`r`n- 任务块高度按时长自动计算（支持 end_date 字段）
...
- **外观**：显示字体（7 种预设 + 系统字体选择器）、字体大小（正常/大/超大三档全局缩放）、10 套完整主题、浅色/深色/跟随系统、三档圆角和三档界面密度
- **通用**：一周起始日、删除确认、**侧边栏显示**（可隐藏低频导航入口；全部任务 / 今日任务 / 设置始终保留；完整侧边栏与折叠图标条同步）
- **通知**：桌面通知、提醒声音
...
- 任务行高 56px、优先级左侧色条
- 打勾弹跳动画、拖拽指示线、细滚动条
- 键盘快捷键：Ctrl+N 新建、**Ctrl+K 命令面板**、Ctrl+F 搜索、Ctrl+1/2/3/4/5 切换视图、?/F1 快捷键帮助、Ctrl+B 折叠侧边栏；支持设置面板自定义快捷键
- **全局命令面板**：Ctrl+K（macOS 兼容 Meta+K）打开，可跳转视图、新建任务、聚焦搜索、打开快捷键帮助，并按标题搜索任务（最多 10 条）
...
- AI 无法直接执行系统命令或读写文件
- 操作指令通过严格权限白名单（创建、修改、完成、创建子任务、删除）；AI 删除任务必须经预览确认，并移入回收站而非直接物理删除
- 所有操作都通过现有 API 接口，受 Tauri 命令白名单限制

## 版本更新记录

### v1.46.0（2026-09-02）- 日历默认视图与自动定位当前时间

- 新增「进入日历时默认显示」设置（日 / 周 / 月）：打开日历模块时按偏好展示，未设置或存储值非法时保持月视图；页面内临时切换视图不回写偏好。
- 日视图、周视图进入时时间轴自动定位到当前本地时间附近（约视口上三分之一处），仅挂载时执行一次，不抢占用户手动滚动；月视图不受影响。
- 不改变任务数据语义、数据库 schema、同步协议或导入导出格式。
...
- 因旧版更新私钥遗失，本版本启用新签名公钥；`v1.45.2` 需要手动安装一次，后续 `v1.45.3+` 可恢复应用内自动更新。
...
- 主题预设从 6 套局部强调色升级为 10 套完整主题：经典蓝、森林绿、薰衣草紫、暖阳橙、樱花粉、莫兰迪、海洋青、石墨灰、纸张米白、午夜蓝。
- 每套主题分别提供浅色和深色的背景、表面、边框、文字、强调色、日历选择态、标题栏、滚动条和阴影变量。
- 设置页新增完整主题缩略图预览，支持浅色、深色和跟随系统；跟随系统切换时无需保持设置页打开。
- 自定义强调色自动选择可读按钮文字，普通文字和强调按钮文字按至少 4.5:1 对比度校验。
- 新增利落、标准、柔和三档圆角，以及紧凑、舒适、宽松三档界面密度，覆盖侧边栏和高频任务行。
- 主题、强调色、圆角和界面密度均可持久化，兼容历史 localStorage key；无效配置自动回退经典蓝。
- 补充主题结构、对比度、持久化、设置面板、窄窗口和全量 E2E 回归测试；不涉及数据库 schema、同步协议、导入导出格式或任务数据行为。

### v1.44.0（2026-07-13）- 日历跨日拖拽创建

- 周视图支持跨日期、跨时间拖拽创建任务，按 15 分钟吸附，支持正向和反向拖动。
- 拖拽过程中按起始日、中间日和结束日显示半透明预览，并显示精确起止时间和持续时长。
- 周视图拖到上下或左右边缘时自动滚动，可继续选择当前不可见的时间和日期。
- 月视图支持跨日期格拖选，松开后预填起止日期和时间，并可在弹窗中编辑标题、备注、优先级、清单及时间范围。
- 跨日任务顶部条使用 `→` 显示精确起止时间；窄窗口支持横向滚动，创建坐标在滚动后保持准确。
- 补充日历范围计算、周/月交互、创建失败保留输入和跨视图契约回归测试；不涉及数据库 schema、同步协议、导入导出格式或主题系统。

### v1.43.2（2026-07-13）- 回收站环数据恢复入口加固

- 修复异常循环 `parent_id`（自环、双环、三环及以上）下，软删除/恢复可能卡死，或 AI 批删表面成功却未写入回收站的问题。
- 修复同次删除的多节点环任务可能被回收站顶层全部隐藏、导致无可点击恢复入口的问题；正常树仍隐藏与父任务同次级联的子任务。
- 前端剪枝、mock 回收站与后端父链判定对齐：环上节点保守保留入口，环外同戳挂接后代可继续隐藏；恢复入口后同戳环同伴仍可连带恢复。
- 补充三节点环回收站/恢复、AI 三环批删、前端环判定等回归测试；不涉及数据库 schema、同步协议、导入导出格式或永久删除。

### v1.43.1（2026-07-13）- 回收站批量删除稳定性修复

- 修复同时批量删除父任务与子任务时，子任务可能获得不同删除时间戳、恢复父任务后仍滞留回收站的问题；前端与 AI 批量删除均会由祖先任务统一执行级联软删除。
- 修复批量删除的双重确认与提前成功提示：仅在用户确认且全部请求完成后显示成功；部分请求失败时会刷新列表、清空过期选择，并提示用户到回收站核对。
- 为级联删除剪枝加入循环父子关系和异常深度防御，避免异常数据导致批量删除卡死或错误地跳过全部任务。
- 补充回收站、父子恢复、AI 乱序批量删除、部分失败状态收敛等前后端回归测试；不涉及数据库 schema、同步协议或永久删除。

### v1.43.0（2026-07-13）- 回收站与可恢复删除

- 删除任务改为可恢复的软删除：任务进入回收站后从普通列表、日历、搜索、统计、提醒和命令面板任务搜索中排除，不再直接物理删除。
- 新增固定回收站视图与命令面板入口，可恢复任务及同次级联删除的子任务；已独立删除的子任务不会被父任务恢复误带回。
- 已删除任务不能再被更新、完成、排序、重复任务完成或 AI 后续动作修改；AI 删除保持预览确认，并统一移入回收站。
- 导入导出与本地数据兼容 `deleted_at`；软删除保留标签、附件、重复规则、完成记录、时间追踪和目标关联。
- 右键菜单新增顶层任务“添加子任务”入口，自动展开并聚焦既有输入框；删除与归档文案、危险操作分区及窄窗口菜单可访问性同步优化。
- 回收站当前不提供永久删除、清空或自动过期清理，优先保障个人任务数据可恢复。

### v1.42.0（2026-07-12）- 全局命令面板与侧边栏导航优化

- 新增全局命令面板：使用 `Ctrl+K`（macOS 兼容 `Meta+K`）快速跳转主要视图、新建任务、聚焦搜索和打开快捷键帮助；可按标题搜索最多 10 条未归档任务。
- 命令面板完善键盘流程：输入框内也可打开，支持 Esc 关闭、上下键选择、Enter 执行、中文输入法组合态保护，以及新建/搜索命令的可靠焦点转移。
- 设置 → 通用新增侧边栏显示控制：可隐藏低频导航入口，完整侧边栏与折叠图标栏同步；全部任务、今日任务和设置始终保留。
- 侧边栏可见性配置持久化到 `dida:sidebar_visible_items`，兼容 legacy 裸 key，并在隐藏当前可选视图时安全回退到全部任务。
- README 清理过期截图占位，改为以功能清单、使用说明和发布说明作为当前功能依据。
