 ...
 * @returns 处理后的任务数据
 */
export function applyDefaultReminder<T extends { due_date?: string; reminder?: string | null }>(
  task: T,
): T {
    ...
    try {
      ...
      if (updates.due_date && !('reminder' in updates)) {
        ...
        if (current?.reminder && current.due_date) {
          ...
          if (
            offset > 0 &&
            isAutoGeneratedReminder(current.reminder, current.due_date, offset)
          ) {
            const newReminder = new Date(
              new Date(updates.due_date).getTime() - offset * 60000,
            ).toISOString()
