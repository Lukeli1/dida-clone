...
export function ListSection({
  ...
  // 右键菜单边界检测
  useLayoutEffect(() => {
    if (!contextMenu || !contextMenuRef.current) { setAdjustedMenuPos(null); return }
  ...
  async function handleDelete(listId: number) {
    setContextMenu(null)
    const ok = await confirm({ title: '删除清单', message: '确定删除此清单吗？清单下的任务将移至收件箱。', danger: true, confirmText: '删除', cancelText: '取消' })
    if (ok) {
      onDeleteList(listId)
      toast.success('清单已删除')
    }
    ...
    <div className="sidebar-lists">
        ...
        <div className="overflow-hidden">
          <div className="mb-2 px-2 space-y-2">
            ...
            <div className="flex gap-2">
              <button onClick={handleCreate} tabIndex={isCreating ? 0 : -1} className="px-3 py-1 text-xs bg-[var(--color-accent)] text-white rounded-md hover:bg-[var(--color-accent-hover)] transition-colors active:scale-[0.97]">创建</button>
            <button onClick={() => { setIsCreating(false); setNewListName('') }} tabIndex={isCreating ? 0 : -1} className="px-3 py-1 text-xs text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-tertiary)] rounded-md transition-colors active:scale-[0.97]">取消</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {contextMenu && (
        <div
          ...
          style={{ left: adjustedMenuPos?.x ?? contextMenu.x, top: adjustedMenuPos?.y ?? contextMenu.y, boxShadow: 'var(--shadow-dropdown)' }}
        >
          <button
      ...
      {showColorPicker !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--color-mask)]" onClick={() => setShowColorPicker(null)}>
          <div className="bg-[var(--color-surface)] rounded-xl p-4" style={{ boxShadow: 'var(--shadow-dropdown)' }} onClick={(e) => e.stopPropagation()}>
