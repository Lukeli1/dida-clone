...
function renderDayView(tasks: Task[], handlers: Partial<React.ComponentProps<typeof DayView>> = {}) {
...
describe('DayView calendar task layout', () => {
  ...
  it('renders multi-day tasks in the all-day area and keeps callbacks working', () => {
    ...
    expect(Array.from(container.querySelectorAll('[data-task]')).some((el) => el.textContent?.includes('跨天日任务'))).toBe(
      false,
    )
