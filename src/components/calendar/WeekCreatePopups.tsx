...
export function WeekCreatePopups({ dateKey, sel, lists, defaultListId }: WeekCreatePopupsProps) {
  ...
  if (createPopup.isQuickAdd) {
    return (
      <div className="absolute z-20 bg-[var(--color-surface)] rounded-lg shadow-xl border border-[var(--color-accent-light)] p-3 w-64"
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <button onClick={cyclePriority} className={`ml-auto p-1 rounded hover:bg-[var(--color-bg-tertiary)] ${priorityFlags[popupPriority].color}`} title={priorityFlags[popupPriority].label}>
  ...
  return (
    <div className="absolute z-20 bg-[var(--color-surface)] rounded-xl shadow-xl border border-[var(--color-border)] p-4 w-72"
      ...
      <input ref={popupInputRef} value={popupTitle} onChange={(e) => setPopupTitle(e.target.value)}
        onKeyDown={(e) => { if (e.key === 'Enter') handlePopupSubmit(); if (e.key === 'Escape') closeCreatePopup() }}
        placeholder="任务标题" className="w-full px-3 py-2 text-sm border border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-text-primary)] rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]/20 focus:border-[var(--color-accent)] mb-2" />
      ...
      <div className="flex gap-2">
        ...
        <button onClick={closeCreatePopup} className="px-3 py-2 text-sm text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-tertiary)] rounded-lg transition-colors">取消</button>
      </div>
    </div>
  )
}
