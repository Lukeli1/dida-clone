...
export function StatsView({ tasks, lists }: StatsViewProps) {
  ...
  const weekTotalSeconds = useMemo(() => timeStats.reduce((sum, s) => sum + s.seconds, 0), [timeStats])
  ...
  const stats = useMemo(() => {
    ...
    const monthCompleted = tasks.filter(
      (t) => t.completed && t.updated_at && isThisMonth(parseISO(t.updated_at)),
    ).length
    ...
    const priorityStats = [
      {
        label: '高',
        value: 1,
        color: PRIORITY_STYLES[1].hex,
        count: tasks.filter((t) => t.priority === 1 && !t.completed).length,
      },
      {
        label: '中',
        value: 2,
        color: PRIORITY_STYLES[2].hex,
        count: tasks.filter((t) => t.priority === 2 && !t.completed).length,
      },
      {
        label: '低',
        value: 3,
        color: PRIORITY_STYLES[3].hex,
        count: tasks.filter((t) => t.priority === 3 && !t.completed).length,
      },
      {
        label: '无',
        value: 0,
        color: PRIORITY_STYLES[0].hex,
        count: tasks.filter((t) => t.priority === 0 && !t.completed).length,
      },
    ]
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            ...
            <StatCard
              ...
              subtitle={`${stats.totalCompleted}/${stats.totalTasks}`}
              color="purple"
            />
          </div>
          ...
          {/* 本周完成趋势 */}
          <div
            className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-6 mb-6"
            ...
            <div className="flex items-end justify-between gap-3 h-48">
              {stats.weekData.map((d, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2">
                  ...
                  <div
                    className="w-full bg-[var(--color-bg-tertiary)] rounded-t-lg overflow-hidden flex items-end"
                    style={{ height: '120px' }}
                  >
                    <div
                  ...
                  <span className="text-xs text-center text-[var(--color-text-secondary)]">
                    {format(d.date, 'EEE', { locale: zhCN })}
                  </span>
                  <span
                    className={`text-xs ${isToday(d.date) ? 'text-[var(--color-accent)] font-bold' : 'text-[var(--color-text-secondary)]'}`}
                  >
                    {format(d.date, 'M/d')}
                  </span>
                </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-[var(--color-text-secondary)]">
                总计{' '}
                <span className="font-semibold text-[var(--color-accent)]">{formatDuration(weekTotalSeconds)}</span>
              </span>
            </div>
                      ...
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm text-[var(--color-text-secondary)] flex items-center gap-2">
                          <span
                            ...
                            style={{ backgroundColor: s.list.color || '#9aa0a6' }}
                          />
                          {s.list.name}
                        </span>
          ...
          {/* P12-05: 历史报告（周报 / 月报） */}
          <div
            className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-6 mt-6"
            style={{ boxShadow: 'var(--shadow-card)' }}
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-[var(--color-text-primary)] flex items-center gap-2">
                  ...
                  {reports.map((r) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span
                          className={`text-xs px-1.5 py-0.5 rounded ${r.type === 'weekly' ? 'bg-[var(--color-accent)]/10 text-[var(--color-accent)]' : 'bg-[var(--color-ai)]/10 text-[var(--color-ai)]'}`}
                        >
                          {r.type === 'weekly' ? '周报' : '月报'}
                        </span>
                    ...
                    <div className="border border-[var(--color-border)] rounded-lg p-4 max-h-96 overflow-y-auto">
                      <div className="flex items-center justify-between mb-3 pb-2 border-b border-[var(--color-border)]">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-xs px-1.5 py-0.5 rounded ${selectedReport.type === 'weekly' ? 'bg-[var(--color-accent)]/10 text-[var(--color-accent)]' : 'bg-[var(--color-ai)]/10 text-[var(--color-ai)]'}`}
                          >
