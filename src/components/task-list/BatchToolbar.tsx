...
export function BatchToolbar({ selectedTaskIds, selectAllTasks, clearSelection, actions, lists }: BatchToolbarProps) {
  ...
  async function handleBatchDelete() {
    const ok = await confirm({ title: '批量删除', message: `确定要删除选中的 ${selectedCount} 个任务吗？`, danger: true, confirmText: '删除', cancelText: '取消' })
    if (ok) {
  ...
  return (
    <div className="mt-2 p-2.5 bg-[var(--color-bg-secondary)] rounded-lg border border-[var(--color-border)] flex items-center gap-2 flex-wrap animate-slide-down">
      ...
      <button onClick={selectAllTasks} className="px-3 py-1.5 text-sm text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-tertiary)] rounded-lg transition-all active:scale-[0.97]">全选</button>
      <button onClick={clearSelection} className="px-3 py-1.5 text-sm text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-tertiary)] rounded-lg transition-all active:scale-[0.97]">取消</button>
      <div className="h-4 w-px bg-[var(--color-bg-tertiary)] mx-1" />
      <button
