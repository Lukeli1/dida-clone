 ...
 */
export function MiniCalendarDropzone({
  currentDate,
  onPrevMonth,
  onNextMonth,
  onDropDate,
  ...
  return (
    <div className="absolute right-6 top-32 z-40 bg-[var(--color-surface)] rounded-lg shadow-2xl border border-[var(--color-border)] p-3 w-72 hidden md:block">
      <div className="flex items-center justify-between mb-2">
        <button
          onClick={onPrevMonth}
          className="p-1 text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)] rounded hover:bg-[var(--color-bg-tertiary)]"
        >
      ...
      <div className="grid grid-cols-7 gap-1 mb-1">
        {weekDays.map((d) => (
          <div key={d} className="text-center text-xs text-[var(--color-text-tertiary)] py-1">
            {d}
          </div>
        ))}
      </div>
