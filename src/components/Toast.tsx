...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const closeExcessToasts = useCallback((newId: number) => {
    ...
    if (idsToClose.length > 0) {
      setToasts((prev) =>
        prev.map((t) => (idsToClose.includes(t.id) ? { ...t, closing: true } : t))
      )
  ...
  return (
    <ToastContext.Provider value={{ toast }}>
      {children}
      <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 pointer-events-none" role="region" aria-label="通知">
        {toasts.map((t) => (
          <div
            key={t.id}
            role="alert"
