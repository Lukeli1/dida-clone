...
> 适用版本：v1.35.2 及以上
> 当前状态：已实现，但暂时封存不启动。
> 封存原因：用户不希望将 GitHub 仓库改为 public；当前 GitHub Releases 更新源依赖客户端无认证访问，private 仓库下无法稳定支撑自动更新检查与下载安装。
> 解封条件：确认可接受公开 Release 分发，或改用私有可访问的更新清单与制品托管方案后，再启用自动更新入口和启动检查。
