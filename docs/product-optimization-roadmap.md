...
> 创建时间：2026-07-09  
> 适用项目：滴答清单复刻  
> 当前版本方向：v1.43.1 已发布；回收站批量删除、父子恢复与异常数据边界已完成稳定性修复
...
`v1.43.1` 已完成发布：回收站链路补齐了父子同批删除与恢复的一致性、AI 乱序批量级联、部分失败后的 UI 状态收敛，以及异常循环父链的防御；不改变软删除、同步或导入导出协议。
...
预期收益：在既有可恢复删除基础上，批量删除父子任务后的恢复一致性得到保障；接口部分失败时不再保留幽灵选择，异常父链也不会冻结批量操作。后续开发可在不牺牲数据安全的前提下逐步完善清理与跨设备一致性。
