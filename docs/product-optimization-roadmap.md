...
> 创建时间：2026-07-09  
> 适用项目：滴答清单复刻  
> 当前版本方向：v1.40.0 模板应用增强已完成
> 下一阶段候选：目标 KR、命令面板与侧边栏设置
...
| 5 | 模板、目标、番茄钟与时间追踪闭环 | `docs/product-optimization-template-goal-pomodoro.md` | v1.40.0 | P1 | ⏳ 部分完成（时间追踪单活跃、番茄钟工时闭环与模板应用增强已完成；KR 待执行） |
...
`docs/product-optimization-template-goal-pomodoro.md` 的阶段 5（目标 KR）。

建议顺序：

1. 目标 KR 数据结构与编辑 UI。
2. 完成后再评估命令面板与侧边栏显示设置。

原因：

- 时间追踪单活跃约束、番茄钟工时入库和模板应用增强已形成闭环；KR 是执行与复盘模块剩余的主要能力缺口。
- KR 会新增持久化结构；先完成 KR 可避免命令面板在目标功能未完成前暴露半成品入口。
- 预期收益：目标进度可通过量化 KR 复盘，模板可直接复用到任意清单、日期、标签和项目文本。

工作量级别：中到大。主要风险是 KR 数据迁移兼容性、目标进度计算与现有任务关联语义的一致性。
