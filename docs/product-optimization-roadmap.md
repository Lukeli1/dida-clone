...
> 创建时间：2026-07-09  
> 适用项目：滴答清单复刻  
> 当前版本方向：v1.44.0 已发布；下一阶段进入 v1.45.0 完整主题系统，继续根据实际使用反馈处理高风险数据边界或高频任务流问题
...
| 11   | 完整主题系统                           | `docs/product-optimization-week-drag-theme.md`            | v1.45.0           | P1     | ⏳ 待执行                                                                                           |
...
`v1.44.0` 已完成发布：周/月视图支持跨日拖拽创建，周视图支持边缘自动滚动和窄窗口横向滚动，创建弹窗可编辑完整起止日期时间；不改变数据库 schema、同步协议、导入导出格式或永久删除策略。

下一步建议：

1. 进入 `v1.45.0` 完整主题系统，先完成主题变量模型、浅色/深色预设和设置页预览。
2. 继续以个人实际使用反馈为依据，只记录会导致数据风险、任务处理绕路或操作不可达的问题。
3. 若后续规划永久删除或自动清理回收站，必须先单独设计同步 tombstone、附件物理清理和跨设备冲突策略，不能直接在现有软删除上追加硬删除。

预期收益：跨日任务从多次打开详情和手动改日期，降低为一次拖选加一次确认；月视图和周视图使用同一创建契约，减少跨视图行为差异。下一阶段主题系统预计可通过完整的背景、文字、边框和日历状态变量，减少长期使用中的视觉疲劳，同时不改变任务数据行为。

工作量级别：主题系统为中等偏大，预计 3～5 个开发日（估算）。主要风险是深浅色对比度、旧组件变量兼容和主题配置持久化，需要分阶段验证。
