...
> 创建时间：2026-07-09  
> 适用项目：滴答清单复刻  
> 当前版本方向：v1.43.2 已发布；回收站环 parent_id 防御与恢复入口可达性已加固
...
`v1.43.2` 已完成发布：在 v1.43.1 批量删除稳定性之上，补齐了任意长度循环 `parent_id` 的删除终止、AI 批删不静默跳过，以及三环及以上同戳任务的回收站恢复入口可达性；不改变软删除、同步或导入导出协议。
...
预期收益：异常/导入脏数据下，删除不再卡死后端锁，AI 批删不会“显示成功却没删”，多节点环任务删除后仍可从回收站恢复。后续开发可在不牺牲数据安全的前提下逐步完善清理与跨设备一致性。
