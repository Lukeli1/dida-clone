...
> 创建时间：2026-07-09  
> 适用项目：滴答清单复刻  
> 当前版本方向：v1.45.1 已完成；后续根据个人实际使用反馈形成新的 PRD
...
`v1.45.1` 已完成：习惯页支持共享历史周导航与历史月历补打，过去日期可补满/撤销，未来日期禁用；今日 `+1/-1` 仍逐次增减并与补打共用忙碌锁。不改变任务数据、数据库 schema、同步协议或导入导出格式。
...
预期收益：用户可从任意历史周补打/撤销习惯，不再只能操作当前周；补打与今日逐次增减互不混淆，且写入失败不会污染本地 records。后续开发将以真实使用反馈为依据。
